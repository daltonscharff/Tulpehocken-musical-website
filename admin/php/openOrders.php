<?php include "../connection.php"; $_SESSION["tab"] = "openOrders.php";?>
<html>
...
<body>
	...
	function search(str){
		if(str == '%'){
			...
			ajaxTableResponse('search_openOrders.php', str, filterNames, filterValues);
			var i=0;
			for(i = 0; i < filterValues.length; i++){
				document.getElementById("cb" + i).checked = "true";
			}
		}else{
			ajaxTableResponse('search_openOrders.php', document.getElementById("searchField").value, filterNames, filterValues);
		}
	}
		...
		<table>
			...
			while($row = mysqli_fetch_assoc($result)){
				...
				<tr style='font-size: .85em'>
					...
					<td>
						<button onclick="ajaxNoResponse('markPaid_openOrders.php?id=<?php echo $row["ID"];?>', 'openOrders.php')">Paid</button>
					</td>
					<td>
						<button onclick="ajaxNoResponse('removeOrder_openOrders.php?id=<?php echo $row["ID"];?>&showName=<?php echo $row["showName"];?>&seats=<?php echo str_replace(' ', '', $row["seats"]);?>', 'openOrders.php')">Remove</button>
					</td>
				</tr>
				<?php
			}
